...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "LITELLM_API_KEY", "LITELLM_MASTER_KEY", "LITELLM_API_BASE", "LITELLM_PORT", "LITELLM_DATABASE_URL",
...
OPTIONAL_ENV_VARS = {
    ...
    "LITELLM_API_KEY": {
        "description": "Client API key for routing Hermes through a LiteLLM proxy",
        "prompt": "LiteLLM client API key",
    ...
    "LITELLM_MASTER_KEY": {
        "description": "LiteLLM admin/master key for local proxy management",
        "prompt": "LiteLLM master key",
        "url": "https://docs.litellm.ai/",
        "password": True,
        "category": "provider",
        "advanced": True,
    ...
    "LITELLM_API_BASE": {
        "description": "LiteLLM proxy base URL override (default: http://localhost:4000/v1)",
        "prompt": "LiteLLM proxy base URL",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "LITELLM_PORT": {
        "description": "Local LiteLLM proxy port for helper scripts and Docker overlay",
        "prompt": "LiteLLM port",
        "url": None,
        "password": False,
        "category": "provider",
    ...
    "LITELLM_DATABASE_URL": {
        "description": "Postgres URL for LiteLLM virtual keys and spend tracking",
        "prompt": "LiteLLM database URL",
        "url": None,
        "password": True,
        "category": "provider",
        "advanced": True,
...
def _normalize_litellm_gateway_config(config: Dict[str, Any]) -> Dict[str, Any]:
    """Expand ``litellm_gateway`` into provider + model alias shims.

    Hermes already knows how to talk to named OpenAI-compatible endpoints via
    ``providers:`` and how to expose stable model names via ``model_aliases:``.
    This helper lets users configure a LiteLLM proxy once and then refer to
    route names like ``orchestrator-default`` or ``research-cheap`` everywhere
    else in config/docs without repeating base URLs.
    """
    gateway = config.get("litellm_gateway")
    ...
    provider_name = str(gateway.get("provider_name") or "litellm").strip().lower()
    if not provider_name:
        provider_name = "litellm"
    api_base = str(gateway.get("api_base") or "").strip()
    api_key_env = str(gateway.get("api_key_env") or "").strip() or "LITELLM_API_KEY"
    default_route = str(gateway.get("default_route") or "").strip()

    if api_base and provider_name not in providers:
        ...
        if default_route:
            provider_entry["default_model"] = default_route
        providers[provider_name] = provider_entry
    elif api_base and provider_name in providers and isinstance(providers[provider_name], dict):
        provider_entry = dict(providers[provider_name])
        provider_entry.setdefault("name", "LiteLLM Gateway")
        provider_entry.setdefault("api", api_base)
        provider_entry.setdefault("key_env", api_key_env)
        if default_route:
            provider_entry.setdefault("default_model", default_route)
        providers[provider_name] = provider_entry

    routes = gateway.get("routes")
    if isinstance(routes, dict):
        for alias, entry in routes.items():
            alias_name = str(alias or "").strip()
            if not alias_name or alias_name in model_aliases:
                continue

            if isinstance(entry, str):
                route_model = entry.strip()
                route_provider = provider_name
                route_base_url = api_base
            elif isinstance(entry, dict):
                route_model = str(entry.get("model") or alias_name).strip()
                route_provider = str(entry.get("provider") or provider_name).strip().lower() or provider_name
                route_base_url = str(entry.get("base_url") or api_base).strip()
            else:
                continue
            ...
            if route_base_url:
                alias_entry["base_url"] = route_base_url
            model_aliases[alias_name] = alias_entry
...
def load_config() -> Dict[str, Any]:
    ...
    normalized = _normalize_litellm_gateway_config(normalized)
...
#
# ── LiteLLM Gateway ────────────────────────────────────────────────────
# Optional OpenAI-compatible proxy layer for centralized model routing,
# provider fallback, and budget controls. Hermes expands this into:
#   - providers.litellm
#   - model_aliases for each named route
#
# Keep upstream provider secrets in ~/.hermes/.env or your service env.
# Agents should only see the LiteLLM client key + route names.
#
# litellm_gateway:
#   enabled: true
#   provider_name: litellm
