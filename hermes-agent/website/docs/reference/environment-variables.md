...
| `SLACK_ENABLED_PROFILES` | Comma-separated Hermes profile names allowed to auto-enable Slack from shared env. Useful when multiple profile gateways share one `.env` and only one should own the Slack Socket Mode connection. |
