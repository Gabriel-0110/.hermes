...
# Docker
./scripts/start-litellm-docker.sh -d
```

This repo now ships a ready-to-edit `litellm_config.yaml` plus `docker-compose.litellm.yml` and helper scripts. The template defines stable route names for agent use:
...
For LiteLLM virtual keys, follow the current LiteLLM requirement set:

1. Set a standard Postgres URL in `LITELLM_DATABASE_URL` or `DATABASE_URL` using `postgresql://...` syntax.
2. Set `LITELLM_MASTER_KEY` to an admin key starting with `sk-`.
3. Start the DB-backed proxy with `./scripts/start-litellm-docker.sh -d`.
4. Generate worker/client keys through `/key/generate` or the helper script:

```bash
scripts/generate-litellm-key.sh orchestrator --write-env ~/.hermes/.env
scripts/generate-litellm-key.sh research --user-id paperclip-research
scripts/generate-litellm-key.sh risk-manager --team-id trading-desk
```

The helper script writes a generated `LITELLM_API_KEY` when `--write-env` is used, and it can attach `user_id` / `team_id` so LiteLLM spend tracking works with `/user/info` and `/team/info`.

Configure Hermes to use LiteLLM as the provider edge:
...
Hermes expands `litellm_gateway` into a named provider plus `model_aliases`, so fallback and smart-routing blocks can use route names instead of provider-native slugs:
...
smart_model_routing:
  ...
  cheap_model:
    provider: litellm
    model: research-cheap
```

For Paperclip or any other OpenAI-compatible runtime, point the client at the same LiteLLM base URL and use the same route names:

```python
from openai import OpenAI

client = OpenAI(
    api_key="sk-litellm-client",
    base_url="http://localhost:4000/v1",
)

response = client.chat.completions.create(
    model="orchestrator-default",
    messages=[{"role": "user", "content": "Review the latest market state."}],
)
```

Keep upstream provider secrets in the backend environment only. Hermes agents should get the LiteLLM client key plus route names, never raw `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `OPENROUTER_API_KEY`, or similar upstream credentials.

The repo template includes JSON logging comments and placeholders for future spend/budget controls inside LiteLLM. For durable spend tracking or per-route quota enforcement, add a real database URL and virtual-key policy on the LiteLLM side rather than pushing budget logic into agents.
