...
def _build_child_agent(
    ...
    child_context_length_override = None
    child_compression_threshold_override = None
    try:
        ...
        raw_delegation_context = delegation_cfg.get("context_length")
        if raw_delegation_context is not None:
            try:
                parsed_context = int(raw_delegation_context)
                if parsed_context > 0:
                    child_context_length_override = parsed_context
            except (TypeError, ValueError):
                logger.warning(
                    "Invalid delegation.context_length=%r; ignoring override",
                    raw_delegation_context,
                )
    except Exception as exc:
        logger.debug("Could not load delegation reasoning_effort: %s", exc)

    parent_context_compressor = getattr(parent_agent, "context_compressor", None)
    if parent_context_compressor is not None:
        raw_threshold = getattr(parent_context_compressor, "threshold_percent", None)
        if isinstance(raw_threshold, (int, float)) and raw_threshold > 0:
            child_compression_threshold_override = float(raw_threshold)

        if child_context_length_override is None:
            parent_model = getattr(parent_agent, "model", None)
            parent_base_url = getattr(parent_agent, "base_url", None)
            if effective_model == parent_model and effective_base_url == parent_base_url:
                raw_context = getattr(parent_context_compressor, "context_length", None)
                if isinstance(raw_context, int) and raw_context > 0:
                    child_context_length_override = raw_context
    ...
    child = AIAgent(
        ...
        config_context_length_override=child_context_length_override,
        compression_threshold_override=child_compression_threshold_override,
    )
