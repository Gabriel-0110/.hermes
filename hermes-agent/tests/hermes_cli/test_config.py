...
class TestLoadConfigDefaults:
    ...
    def test_litellm_gateway_synthesizes_provider_and_aliases(self, tmp_path):
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            config_path = tmp_path / "config.yaml"
            ...
            config = load_config()
            assert config["providers"]["litellm"]["api"] == "http://localhost:4000/v1"
            assert config["providers"]["litellm"]["key_env"] == "LITELLM_API_KEY"
            assert config["providers"]["litellm"]["default_model"] == "orchestrator-default"
            assert config["model_aliases"]["orchestrator-default"]["provider"] == "litellm"
            assert config["model_aliases"]["orchestrator-default"]["model"] == "orchestrator-default"
            assert config["model_aliases"]["research-cheap"]["base_url"] == "http://localhost:4000/v1"

    def test_litellm_gateway_does_not_override_existing_aliases(self, tmp_path):
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            config_path = tmp_path / "config.yaml"
            config_path.write_text(
                yaml.safe_dump(
            ...
            config = load_config()
            assert config["model_aliases"]["research-cheap"]["model"] == "custom-cheap"
            assert config["model_aliases"]["research-cheap"]["base_url"] == "http://localhost:9999/v1"
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_litellm_provider_vars_registered(self):
        from hermes_cli.config import OPTIONAL_ENV_VARS

        assert "LITELLM_API_KEY" in OPTIONAL_ENV_VARS
        assert OPTIONAL_ENV_VARS["LITELLM_API_KEY"]["category"] == "provider"
        assert OPTIONAL_ENV_VARS["LITELLM_MASTER_KEY"]["password"] is True
        assert OPTIONAL_ENV_VARS["LITELLM_API_BASE"]["password"] is False
        assert OPTIONAL_ENV_VARS["LITELLM_DATABASE_URL"]["password"] is True
