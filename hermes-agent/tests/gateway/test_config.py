...
class TestSlackProfileEnvOverrides:
    def test_slack_env_enabled_for_matching_profile(self):
        config = GatewayConfig()
        ...
        with patch.dict(
            os.environ,
            {
                ...
                "SLACK_ENABLED_PROFILES": "risk-manager,orchestrator",
            },
            clear=True,
        ), patch(
            "hermes_cli.profiles.get_active_profile_name",
            return_value="risk-manager",
        ):
            _apply_env_overrides(config)
    ...
    def test_slack_env_skipped_for_non_matching_profile(self):
        config = GatewayConfig()
        ...
        with patch.dict(
            ...
            {
                "SLACK_BOT_TOKEN": "xoxb-test",
                "SLACK_ENABLED_PROFILES": "risk-manager",
            },
            clear=True,
        ), patch(
            "hermes_cli.profiles.get_active_profile_name",
            return_value="market-researcher",
        ):
            _apply_env_overrides(config)

        assert Platform.SLACK not in config.platforms

    def test_profile_allowlist_defaults_to_enabled_when_unset(self):
        with patch.dict(os.environ, {}, clear=True):
            assert _profile_is_enabled_for_env("SLACK_ENABLED_PROFILES") is True
