...
def _make_mock_parent(depth=0):
    ...
    parent.context_compressor = MagicMock()
    parent.context_compressor.context_length = 400_000
    parent.context_compressor.threshold_percent = 0.9
    return parent
...
class TestDelegateTask(unittest.TestCase):
    ...
    def test_child_inherits_parent_context_window_and_threshold(self, _mock_cfg):
        parent = _make_mock_parent()
        ...
        with patch("run_agent.AIAgent") as MockAgent:
            ...
            self.assertEqual(kwargs["config_context_length_override"], 400_000)
            self.assertEqual(kwargs["compression_threshold_override"], 0.9)

    @patch("tools.delegate_tool._load_config", return_value={"context_length": 262144})
    def test_delegation_context_length_override_reaches_child(self, _mock_cfg):
        parent = _make_mock_parent()
        ...
        with patch("run_agent.AIAgent") as MockAgent:
            ...
            _, kwargs = MockAgent.call_args
            self.assertEqual(kwargs["config_context_length_override"], 262144)
            self.assertEqual(kwargs["compression_threshold_override"], 0.9)
