...
class TestGatewayCleanupWiring:
    ...
    def test_gateway_stop_calls_close(self):
        ...
        runner._stop_task = None        # prevent MagicMock from blocking `await`
        runner._restart_requested = False  # prevent detached-restart branch
        runner._restart_drain_timeout = 5  # numeric timeout expected by stop()
        # _drain_active_agents must be async and return the agents to finalize
        runner._drain_active_agents = AsyncMock(
            return_value=(runner._running_agents, False)
        )
        # Bind the real _finalize_shutdown_agents so it actually calls close()
        # on agents — MagicMock auto-creates a stub that does nothing.
        from gateway.run import GatewayRunner as _GWR
        runner._finalize_shutdown_agents = lambda agents: _GWR._finalize_shutdown_agents(runner, agents)
