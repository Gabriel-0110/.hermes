...
class GatewayRunner:
    ...
    def _refresh_turn_routing_config(self) -> None:
        """Reload hot-path routing config so live gateways pick up edits.

        Gateway startup caches fallback and smart-routing settings, but those
        values directly influence cached ``AIAgent`` construction.  If the user
        edits ``config.yaml`` while the gateway is running, subsequent turns can
        keep using stale fallback chains unless we refresh the settings before
        routing and agent-cache lookup.
        """
        try:
            self._fallback_model = self._load_fallback_model()
        except Exception:
    ...
    async def _run_agent(
        ...
        def run_sync():
            ...
            self._refresh_turn_routing_config()
